...
async function init() {
    ...
    if (!isLocalhost) console.log('☁️  Detectado ambiente Nuvem – habilitando SSL.');
    ...
    try {
        ...
        await client.query('SELECT count(*) FROM waiting_tickets')
            .then(res => console.log(`📊 Status: ${res.rows[0].count} senhas registradas no banco.`))
            .catch(err => {
                if (err.code === '42P01') {
                    console.warn('⚠️  ALERTA: tabela "waiting_tickets" não existe. Crie-a via SQL editor.');
                } else {
                    console.warn('⚠️  Aviso ao verificar tabelas:', err.message);
                }
            });
        client.release();
        dbReady = true;
        ...
        console.error('   → Verifique a senha no .env ou o status do projeto Supabase.');
...
function startServer() {
    app.listen(port, () => {
        ...
        if (connectionString) console.log('🔗 Conectando ao banco... aguarde.');
    });
}
    ...
    try {
        if (dbReady) {
            const result = await pool.query(
                'SELECT id, username, full_name, email, cpf, profile_picture, role, created_at FROM users WHERE username = $1 AND password = $2',
                [username, password]
            );
...
app.post('/api/called-history', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: 'Erro interno ao registrar histórico.' });
    }
});
...
app.get('/api/completed-services', async (req, res) => {
    ...
    try {
        ...
        console.error('Erro ao buscar serviços completados:', error);
...
app.post('/api/completed-services', async (req, res) => {
    ...
    try {
        ...
        console.error('Erro ao registrar serviço completado:', error);
...
app.delete('/api/abandoned-tickets/:ticketNumber', async (req, res) => {
    ...
    try {
        if (dbReady) {
            ...
            if (result.rowCount === 0) {
                return res.status(404).json({ error: 'Senha abandonada não encontrada.' });
            }
